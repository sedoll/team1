...
<%
    ...
    try {
        ...
        if(cnt > 0) {
            ...
            out.println("<script>location.href='/pro01/qna_career/getQna.jsp?qno="+ qno +"'</script>");
        } else {
            System.out.println("답변 생성 실패");
//            response.sendRedirect("addTeaAns.jsp");
            out.println("<script>alert('답변 추가 실패')</script>");
            out.println("<script>location.href='/pro01/qna_career/addTeaAns.jsp?qno="+ qno +"'</script>");
        }
