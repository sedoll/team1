...
<%
    ...
    try {
        conn = con.connect();
        String sql = "INSERT INTO qna_problem(title, content, author, par) VALUES(?, ?, ?, ?) ";
    ...
    try {
        conn = con.connect();
        String sql = "update qna_problem set par=qno where par=0 and lev=0";
        pstmt = conn.prepareStatement(sql);
        int cnt = pstmt.executeUpdate();
        if(cnt > 0) {
...
//            response.sendRedirect("qnaList.jsp");
            ...
            out.println("<script>location.href='/pro01/qna_problem/qnaList.jsp'</script>");
        } else {
