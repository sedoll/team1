...
<%
    ...
    try {
        ...
        if(cnt > 0) {
            ...
            out.println("<script>location.href='/pro01/board_tea/getBoardTea.jsp?bno="+ bno +"'</script>");
        } else {
            System.out.println("댓글 생성 실패");
//            response.sendRedirect("addTeaAns.jsp");
            out.println("<script>alert('댓글 추가 실패')</script>");
            out.println("<script>location.href='/pro01/board_tea/addTeaAns.jsp?bno="+ bno +"'</script>");
        }
